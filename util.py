...
class WCT(nn.Module):
    ...
    def transform(self,cF,sF,csF,alpha,return_whiten=False):
        cF = cF.double()
        sF = sF.double()
        # if(self.args.do_patches):
        #     csF = torch.zeros_like(cF)
        #     for i in cF.shape[0]:
        #         C,W,H = cF[i].size(0),cF[i].size(1),cF[i].size(2)
        #         _,W1,H1 = sF.size(0),sF.size(1),sF.size(2)
        #         cFView = cF.view(C,-1)
        #         sFView = sF.view(C,-1)

        #         targetFeature = self.whiten_and_color(cFView,sFView)
        #         targetFeature = targetFeature.view_as(cF[i])
        #         ccsF = alpha * targetFeature + (1.0 - alpha) * cF[i]
        #         ccsF = ccsF.float().unsqueeze(0)
        #         csF[i].data.resize_(ccsF.size()).copy_(ccsF)
        #     return csF
        ...
        targetFeature,whiten_cF = self.whiten_and_color(cFView,sFView)
        targetFeature = targetFeature.view_as(cF)
        ccsF = alpha * targetFeature + (1.0 - alpha) * cF
        ccsF = ccsF.float().unsqueeze(0)
        csF.data.resize_(ccsF.size()).copy_(ccsF)
        if(return_whiten==True):
            return csF,whiten_cF
        else:
            return csF
